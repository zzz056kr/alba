  ...
  const { mutate: login, isPending } = useMutation({
    ...
    onSuccess: (res) => {
      if (res.data) {
        ...
        toast.success("로그인 성공! 환영합니다!", { duration: 1000 })
        router.push("/")
      }
    },
    onError: () => {},
  ...
  return (
    <div className="flex flex-1 items-center justify-center bg-background py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <Form {...form}>
          <form className="mt-8 space-y-6" onSubmit={form.handleSubmit((v) => login(v))}>
            ...
            <div className="space-y-3">
              <Button type="button" variant="outline" onClick={() => handleSocialLogin('Google')} className="w-full h-10">
              ...
              <Button type="button" onClick={() => handleSocialLogin('Kakao')} className="w-full h-10 bg-yellow-400 text-gray-900 hover:bg-yellow-500">
                <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="#3C1E1E">
                  <path d="M12 3c5.799 0 10.5 3.664 10.5 8.185 0 4.52-4.701 8.184-10.5 8.184a13.5 13.5 0 0 1-1.727-.11l-4.408 2.883c-.501.265-.678.236-.472-.413l.892-3.678c-2.88-1.46-4.785-3.99-4.785-6.866C1.5 6.665 6.201 3 12 3z"/>
                </svg>
                카카오로 로그인
              </Button>
              <Button type="button" onClick={() => handleSocialLogin('Naver')} className="w-full h-10 bg-green-500 text-white hover:bg-green-600">
