CREATE TABLE t_account (
    ...
    status                  ENUM('PENDING', 'ACTIVE', 'INACTIVE', 'DELETED') NOT NULL COMMENT '상태',
...
CREATE TABLE t_token (
    ...
    access_expires_at       DATETIME NOT NULL COMMENT '액세스 토큰 만료 시간',
    refresh_expires_at      DATETIME NOT NULL COMMENT '리프레시 토큰 만료 시간',
    ...
    revoked_at              DATETIME NULL COMMENT '토큰 무효화 시간',
...
CREATE TABLE t_shop (
    ...
    address                 VARCHAR(255) NULL COMMENT '매장 주소',
    invite_code             VARCHAR(10) NOT NULL UNIQUE COMMENT '합류 초대 코드 (예: X7A9KQ)',
    qr_code_value           VARCHAR(36) NOT NULL UNIQUE COMMENT '출퇴근 인증용 고유 QR UUID',
    latitude                DECIMAL(10, 7) NOT NULL COMMENT '매장 중심 GPS 위도 (기준점)',
    longitude               DECIMAL(10, 7) NOT NULL COMMENT '매장 중심 GPS 경도 (기준점)',
    status                  ENUM('ACTIVE', 'CLOSED', 'SUSPENDED') NOT NULL DEFAULT 'ACTIVE' COMMENT '매장 상태',
    created_no              BIGINT NULL COMMENT '생성자 (최초 생성 사장님 account_no)',
    created_at              TIMESTAMP DEFAULT CURRENT_TIMESTAMP() NOT NULL COMMENT '생성일시',
    modified_no             BIGINT NULL COMMENT '수정자',
    modified_at             TIMESTAMP DEFAULT CURRENT_TIMESTAMP() NOT NULL ON UPDATE CURRENT_TIMESTAMP() COMMENT '수정일시',
    INDEX idx_shop_invite_code (invite_code),
    INDEX idx_shop_qr_code (qr_code_value)
...
CREATE TABLE t_shop_member (
    ...
    shop_role               ENUM('OWNER', 'MANAGER', 'STAFF') NOT NULL COMMENT '매장 내 역할',
    base_wage               INT NULL COMMENT '기본 시급',
    is_app_user             CHAR(1) NOT NULL DEFAULT 'Y' COMMENT '앱 가입자 여부 (Y: 앱유저, N: 사장수기등록)',
    employment_type         ENUM('REGULAR', 'DAILY') NOT NULL DEFAULT 'REGULAR' COMMENT '고용 형태 (정직원, 일일대타)',
    status                  ENUM('PENDING', 'ACTIVE', 'INACTIVE', 'QUIT') NOT NULL DEFAULT 'PENDING' COMMENT '상태',
...
CREATE TABLE t_schedule (
    ...
    status                  ENUM('SCHEDULED', 'CANCELED') NOT NULL DEFAULT 'SCHEDULED' COMMENT '스케줄 상태',
    created_no              BIGINT NULL COMMENT '생성자',
    created_at              TIMESTAMP DEFAULT CURRENT_TIMESTAMP() NOT NULL COMMENT '생성일시',
    modified_no             BIGINT NULL COMMENT '수정자',
    modified_at             TIMESTAMP DEFAULT CURRENT_TIMESTAMP() NOT NULL ON UPDATE CURRENT_TIMESTAMP() COMMENT '수정일시',
...
CREATE TABLE t_attendance (
    ...
    clock_in_at             DATETIME NOT NULL COMMENT '실제 출근 타각 일시',
    clock_in_lat            DECIMAL(10, 7) NULL COMMENT '출근 타각 시 앱에서 보낸 위도',
    clock_in_lng            DECIMAL(10, 7) NULL COMMENT '출근 타각 시 앱에서 보낸 경도',
    clock_out_at            DATETIME NULL COMMENT '실제 퇴근 타각 일시',
    clock_out_lat           DECIMAL(10, 7) NULL COMMENT '퇴근 타각 시 앱에서 보낸 위도',
    clock_out_lng           DECIMAL(10, 7) NULL COMMENT '퇴근 타각 시 앱에서 보낸 경도',
    auth_type               ENUM('QR_GPS', 'MANUAL') NOT NULL COMMENT '인증 방식 (QR스캔+GPS확인, 사장수기입력)',
    clock_in_status         ENUM('NORMAL', 'LATE', 'EXCEPTION') NULL COMMENT '출근 상태 (정상, 지각, 위치예외)',
    clock_out_status        ENUM('NORMAL', 'EARLY_LEAVE', 'EXCEPTION') NULL COMMENT '퇴근 상태 (정상, 조퇴, 위치예외)',
    created_no              BIGINT NULL COMMENT '생성자',
    created_at              TIMESTAMP DEFAULT CURRENT_TIMESTAMP() NOT NULL COMMENT '생성일시',
    modified_no             BIGINT NULL COMMENT '수정자',
    modified_at             TIMESTAMP DEFAULT CURRENT_TIMESTAMP() NOT NULL ON UPDATE CURRENT_TIMESTAMP() COMMENT '수정일시',
